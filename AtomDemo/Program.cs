...
namespace AtomDemo
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //SerializerAllAtom();
            //DeSerializerAllAtom();
            SerializerOneAtomUsingJSON(Atom.Parse("Hydrogen 1 0 1.0079 H"), "oneatomjson.json");
            DeSerializeOneAtomUsingJSON("oneatomjson.json");
            SerializerAllAtomUsingJSON(elements, "allatom.json");
            DeSerializeAllAtomUsingJSON("allatom.json");

            //  add the following statement in global scope
            ////To create a collection to work with
            //foreach (var iteom in elements)
            //{
            //    Console.WriteLine(item);
            //}
        }
        static void SerializerOneAtom()
        {
            XmlSerializer serializer = new XmlSerializer(typeof(Atom));
            TextWriter writer = new StreamWriter("OneAtom.xml");
            serializer.Serialize(writer, Atom.Parse("Hydrogen 1 0 1.0079 H"));
            writer.Close();
        }

        static void DeSerializerOneAtom()
        {
            XmlSerializer serializer = new XmlSerializer(typeof(Atom));
            TextReader reader = new StreamReader("OneAtom.xml");
            Atom myAtom = (Atom)serializer.Deserialize(reader);
            Console.WriteLine(myAtom);
            //serializer.Serialize(reader, Atom.Parse("Hydrogen 1 0 1.0079 H"));
            reader.Close();
        }
        static void SerializerAllAtom()
        {
            XmlSerializer serializer = new XmlSerializer(typeof(List<Atom>));

            TextWriter writer = new StreamWriter("OneAtom.xml");
            /*foreach (var item in elements) //for text file for all items 
            {
        ...
        static void DeSerializerAllAtom()
        {
            XmlSerializer serializer = new XmlSerializer(typeof(List<Atom>));
            TextReader reader = new StreamReader("AllAtom.xml");

            List<Atom> allAtom = (List<Atom>)serializer.Deserialize(reader);
            foreach (var item in allAtom)
            {
                Console.WriteLine(item);
            }
        ...
        static void SerializerOneAtomUsingJSON(Atom atom, string filename)
        {

            using (TextWriter writer = new StreamWriter(filename))
            {
                JavaScriptSerializer serializer = new JavaScriptSerializer();
                writer.WriteLine(serializer.Serialize(atom));
            }

        }
        static void SerializerAllAtomUsingJSON(List<Atom> atoms, string filename)
        {

            using (TextWriter writer = new StreamWriter(filename))
            {
                JavaScriptSerializer serializer = new JavaScriptSerializer();
                writer.WriteLine(serializer.Serialize(atoms));
            }

        }
        static void DeSerializeOneAtomUsingJSON(string filename)
        {
            using (TextReader reader = new StreamReader(filename))
            {
                JavaScriptSerializer serializer = new JavaScriptSerializer();
                Console.WriteLine(serializer.Deserialize<Atom>(reader.ReadToEnd()));
            }
        }

        static void DeSerializeAllAtomUsingJSON(string filename)
        {
            using (TextReader reader = new StreamReader(filename))
            {
                JavaScriptSerializer serializer = new JavaScriptSerializer();
                List<Atom> atoms = serializer.Deserialize<List<Atom>>(reader.ReadToEnd());
        ...
        static void Initializer()
        {
            ...
            elements.Add(Atom.Parse("Hassium 108 169 277 Hs Extra Tokens"));
        }
        ...
        //9 - Read the above file and display all of the items


        //}
        //public static Atom DisplayAll()
        // {
